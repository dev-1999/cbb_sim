...
def predict(i, num_matches = 5, distance_weight = 2, num_comps = 30):
    info_dict = {}
...
def historical_page(team, df_curr):
    ...
    if is_historical:
        ...
        denom = 0
        for i in range(30):
            if ret.loc[i,'S'] != "None" and ret.loc[i,'S'] != 17:
                avg_seed += ret.loc[i,'S'] * 1/ret.loc[i,'distance']**2
                denom += 1/ret.loc[i,'distance']**2
        if avg_seed < 1:
            avg_seed = "None"
        else:
